...
counts_df = load_counts('counts.txt')
...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Graph(id='counts-plot', style={'height': '600px'})
...
def update_plot_and_stats(x_axis, y_axis, z_axis, color_by_count, size_by_count,
    ...
    stats = html.Div([
        ...
        html.P(f"Min/Max count filters: {min_count if min_count is not None else '-'} / {max_count if max_count is not None else '-'}")
    ])
